...
enemy = pygame.image.load(
    "C:/Users/yhmsm/workspace/nadocoding/ut1_pygame_python/enemy.png")
enemy_size = enemy.get_rect().size  # 이미지의 크기를 구해옴
enemy_width = enemy_size[0]  # 캐릭터의 가로 크기
enemy_height = enemy_size[1]  # 캐릭터의 세로 크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2)  # 가로 크기 계산후 위치
...
# 시작 시간
start_ticks = pygame.time.get_ticks()  # 현재 tick을 받아옴 -> 시작 시간을 받아옴

# 이벤트 루프
running = True  # 게임이 진행중인가?
while running:
    dt = clock.tick(60)  # dt: 델타 ??? 설정만 하는데 왜 바뀌지
    ...
    for event in pygame.event.get():  # 어떤 이벤트가 발생했는가?
        ...
        if event.type == pygame.KEYDOWN:  # 키가 눌러졌는지 확인
            if event.key == pygame.K_LEFT:  # 캐릭터 왼쪽
                ...
                to_x += character_speed
            elif event.key == pygame.K_UP:  # 캐릭터 위쪽
                to_y -= character_speed
            elif event.key == pygame.K_DOWN:  # 캐릭터 아래쪽
                to_y += character_speed

        if event.type == pygame.KEYUP:  # 방향키를 떼면 멈춤 - 눌르다가 떼는 경우
    ...
    # 캐릭터의 좌표를 변경
    # 프레임이 달라진다고 게임 자체의 속도가 바뀌면 안되기 때문에 보정 (*dt)
    ...
    # 충돌 체크 - colliderect() 함수: 충돌 한 경우
    if character_rect.colliderect(enemy_rect):
        print("충돌 했어요")
        running = False  # 게임을 종료함
    ...
    # 경과 시간(ms)을 1000으로 나눈 초(s) 단위로 표시
